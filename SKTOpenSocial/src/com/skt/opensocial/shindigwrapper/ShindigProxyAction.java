...
public class ShindigProxyAction extends CommonBaseAction {
	...
	public void init(ServletConfig servletConfig) {
		...
		logger.info("proxyHost=" + stringProxyHostNew);
		...
		if (stringProxyPortNew != null && stringProxyPortNew.length() > 0) {
			...
			logger.info("proxyPort=" + stringProxyPortNew);
		}
		...
		if (stringProxyPathNew != null && stringProxyPathNew.length() > 0) {
			...
			logger.info("proxyPath=" + stringProxyPathNew);
		}
	...
	private void handleStandardPost(PostMethod postMethodProxyRequest,
		...
		for (String stringParameterName : mapPostParameters.keySet()) {
			...
			for (String stringParamterValue : stringArrayParameterValues) {
				...
				NameValuePair nameValuePair = new org.apache.commons.httpclient.NameValuePair(
	...
	private void executeProxyRequest(HttpMethod httpMethodProxyRequest,
			...
			HttpServletResponse httpServletResponse) throws IOException,
			ServletException {
		logger.info("Shindig proxy request started");
		...
		inputStreamProxyResponse = httpMethodProxyRequest
				.getResponseBodyAsStream();
		BufferedInputStream bufferedInputStream = new BufferedInputStream(
				inputStreamProxyResponse);
		outputStreamClientResponse = httpServletResponse
				.getOutputStream();
		//
		...
		logger.info("Shindig proxy request finished");
	...
	private String getProxyURL(HttpServletRequest httpServletRequest) {
		...
		logger.info("proxyURL 0=" + stringProxyURL);
		...
		if (!this.getProxyPath().equalsIgnoreCase("")) {
			...
			logger.info("proxyURL 1=" + stringProxyURL);
		}
		// Handle the path given to the servlet
		if(httpServletRequest.getPathInfo() != null) {
			stringProxyURL += httpServletRequest.getPathInfo();
			logger.info("proxyURL 2=" + stringProxyURL);
		}
		// Handle the query string
		if (httpServletRequest.getQueryString() != null) {
			stringProxyURL += "?" + httpServletRequest.getQueryString();
		}
		logger.info("proxyURL final=" + stringProxyURL);
		return stringProxyURL;
	}
	private InputStream inputStreamProxyResponse;
	private OutputStream outputStreamClientResponse;
	
	public InputStream getInputStream() {
		return inputStreamProxyResponse;
	}
//
	public OutputStream getOutputStream(){
		return outputStreamClientResponse;
	}
