	...
	public String execute(){
		...
		try {
			...
			if(registerType.equals(GadgetRegisterTypeConstants.URL)){
				...
				// do nothing here, but JSP should have some logic to handle the source
				String developerId = gadget.getDeveloper().getId();
				Long gadgetId = gadget.getId();
				
				String realDirPath = ServletActionContext.getServletContext().getRealPath("/gadget");
				String gadgetXmlFileName = developerId + "-" + gadgetId + ".xml";
				
				File gadgetXmlFile = new File(realDirPath + "/" + gadgetXmlFileName);
				...
				gadgetXmlUrlBuff.append(ServletActionContext.getRequest().getServerName());
				gadgetXmlUrlBuff.append(":");
				gadgetXmlUrlBuff.append(ServletActionContext.getRequest().getServerPort());
				//gadgetXmlUrlBuff.append("/");
				gadgetXmlUrlBuff.append(ServletActionContext.getRequest().getContextPath());
